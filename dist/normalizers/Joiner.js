...
class Joiner {
    ...
    joinThem() {
        ...
        for (let i = 0; i < subjectsList.length; i++) {
            ...
            let { key } = current;
            let [apuntes, actividades] = this.searchCoincidences(key, materialsList);
            subjectsList[i].setMaterials(apuntes.length === 1 ? apuntes[0] : apuntes, actividades.length === 1 ? actividades[0] : actividades);
        }
        return subjectsList;
    }
    /**
